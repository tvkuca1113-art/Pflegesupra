...
export default function Footer() {
  return (
    <footer className="on-dark mt-auto bg-[var(--color-brand-deep)] text-white">
      <div className="shell py-12 pb-28 lg:pb-12">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.15fr)_repeat(3,minmax(0,1fr))]">
          ...
          <div>
            <h2 className="text-[var(--text-xl)] text-white">Direkt erreichen</h2>
            <span className="horizont mt-3" aria-hidden="true" />
            <ul className="m-0 list-none space-y-3 p-0">
              <li>
                <a
                  href={business.phone.href}
                  className="inline-flex items-center gap-2.5 text-[var(--text-lg)] font-bold text-white underline decoration-2 underline-offset-4"
              ...
              <li className="flex items-start gap-2.5 text-white/85">
                ...
                <span>
                  ...
                  <span className="mt-1 block text-[var(--text-sm)] text-white/70">
                    Pflegeeinsätze finden auch außerhalb dieser Zeiten statt.
                  </span>
                </span>
              </li>
            </ul>
            <p className="mt-5 text-[var(--text-sm)] text-white/70">
              Fax {business.fax}
            </p>
          </div>
          ...
          {footerNav.map((col) => (
            <nav key={col.heading} aria-label={col.heading}>
              <h2 className="text-[var(--text-lg)] text-white">{col.heading}</h2>
              <span className="horizont mt-3" aria-hidden="true" />
              <ul className="m-0 list-none space-y-2.5 p-0">
                {col.links.map((l) => (
                  <li key={l.href}>
                    <Link href={l.href} className="text-white/90 underline decoration-1 underline-offset-4 hover:text-white hover:decoration-2">
                      {l.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-10 flex flex-col gap-4 border-t border-white/20 pt-6 text-[var(--text-sm)] text-white/75 sm:flex-row sm:items-center sm:justify-between">
          <p className="m-0">
            {business.legalName} · Inhaber {business.ownerShort} · {business.approval}
          </p>
          <p className="m-0 flex gap-5">
            <Link href="/impressum" className="text-white/90 underline underline-offset-4">Impressum</Link>
            <Link href="/datenschutz" className="text-white/90 underline underline-offset-4">Datenschutz</Link>
          </p>
        </div>
      </div>
    </footer>
