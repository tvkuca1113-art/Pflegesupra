...
export function PageHeader({
  ...
  return (
    <div className="section--tight border-b border-[var(--color-line)] bg-[var(--color-paper)] pt-8">
      <div className="shell">
        {eyebrow ? <p className="eyebrow">{eyebrow}</p> : null}
        <span className="horizont" aria-hidden="true" />
        <h1 className="max-w-[20ch] text-[var(--text-4xl)] sm:text-[var(--text-5xl)]">{title}</h1>
        {intro ? (
          <p className="measure mt-5 text-[var(--text-lg)] text-[var(--color-ink-muted)]">{intro}</p>
        ) : null}
        {children}
      </div>
    </div>
...
export function Breadcrumbs({ trail }: { trail: { name: string; path: string }[] }) {
  return (
    <nav aria-label="Brotkrumennavigation" className="shell pt-5">
      <ol className="m-0 flex list-none flex-wrap items-center gap-1 p-0 text-[var(--text-sm)]">
        {trail.map((t, i) => (
          <li key={t.path} className="flex items-center gap-1">
            {i > 0 ? <IconChevron className="-rotate-90 text-[var(--color-line-strong)]" aria-hidden="true" /> : null}
            {i === trail.length - 1 ? (
              <span aria-current="page" className="text-[var(--color-ink-muted)]">{t.name}</span>
            ) : (
...
export function SectionHead({
  ...
  return (
    <div className="mb-8">
      {eyebrow ? <p className="eyebrow">{eyebrow}</p> : null}
      <span className="horizont" aria-hidden="true" />
      <As className="max-w-[22ch] text-[var(--text-3xl)]">{title}</As>
      {intro ? <p className="measure mt-4 text-[var(--text-lg)] text-[var(--color-ink-muted)]">{intro}</p> : null}
    </div>
  );
}
...
export function CtaBand({
  ...
  return (
    <section className="on-dark bg-[var(--color-brand-deep)] py-14 text-white">
      <div className="shell grid items-center gap-8 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
        <div>
          <span className="horizont" aria-hidden="true" />
          <h2 className="max-w-[24ch] text-[var(--text-3xl)] text-white">{title}</h2>
          <p className="measure mt-4 text-[var(--text-lg)] text-white/85">{body}</p>
        </div>
        <div className="flex flex-col gap-3">
          <a href={business.phone.href} className="btn btn--onDark text-[var(--text-lg)]">
            <IconPhone />
          ...
          <p className="m-0 text-[var(--text-sm)] text-white/70">
...
export function LinkCard({
  ...
  return (
    <li className="group relative flex flex-col border-t-4 border-[var(--color-line)] bg-white pt-4 transition-colors hover:border-[var(--color-sun)]">
      <h3 className="text-[var(--text-xl)]">
        <Link href={href} className="no-underline after:absolute after:inset-0 after:content-['']">
          {title}
        </Link>
      </h3>
      <p className="mt-2 flex-1 text-[var(--color-ink-muted)]">{body}</p>
      {meta ? <p className="mt-3 text-[var(--text-sm)] text-[var(--color-ink-muted)]">{meta}</p> : null}
